...
class RouteImpl extends Route {
  ...
  void addRoute(
    ...
    var route = new RouteImpl._new(
        name: name,
        path: matcher as UrlMatcher,
        parent: this,
        dontLeaveOnParamChanges: dontLeaveOnParamChanges as bool,
        pageTitle: pageTitle,
  ...
  String _getTailUrl(Route routeToGo, Map parameters) {
    var tail = '';
    for (RouteImpl route = routeToGo as RouteImpl;
        route != this;
        route = route.parent) {
...
class Router {
  ...
  Future<bool> route(String path,
    ...
    // Alex: cast by copy list
    var future = _preLeave(path, treePath, new List.from(trimmedActivePath),
  ...
  Future<bool> _preEnter(
    ...
    return Future.wait(preEnterFutures)
        .then/*<Future<bool>>*/((List<bool> results) {
  ...
  String url(String routePath,
    ...
    return (_useFragment ? '#' : '') +
        ((baseRoute as dynamic)._getHead(tail) as String) +
        _buildQuery(queryParameters);
  }
  ...
  _Match _getMatch(Route route, String path) {
    ...
    // default route
    if (match == null) {
      return new _Match(route as RouteImpl, new UrlMatch('', '', {}), {});
    }
    return new _Match(route as RouteImpl, match, _parseQuery(route, path));
  }
